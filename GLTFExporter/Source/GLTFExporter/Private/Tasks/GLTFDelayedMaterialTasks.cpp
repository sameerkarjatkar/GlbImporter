...
void FGLTFDelayedMaterialTask::Process()
{
	...
	// Get the extra info 
	// CustomProperty 
	UCustomProperty* pUCustomProperty = UCustomProperty::GetExtrasInfo(const_cast<UMaterial*>(BaseMaterial));
	if (pUCustomProperty)
	{
		TSharedPtr<FJsonObject> extrasObject = pUCustomProperty->EProperty.ExtrasInfo;
		FGLTFJsonCustomProperty customProperty;
		customProperty.PropertyExtraData = extrasObject;
		JsonMaterial->CustomProperties.Add(customProperty);
	}
	else
	{
		//Log
