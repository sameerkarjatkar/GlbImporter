...
void FGLTFDelayedStaticMeshTask::Process()
{
	...
	// Extras Property
	UCustomProperty* pUCustomProperty = UCustomProperty::GetExtrasInfo(const_cast<UStaticMesh*>(StaticMesh));
	if (pUCustomProperty)
	{
		//ExtrasInfo = pUCustomProperty->EProperty.ExtrasInfo;
		//JsonMesh->ExtrasInfo = pUCustomProperty->EProperty.ExtrasInfo; //Sameer commented out
		FGLTFJsonCustomProperty customProperty;
		
		customProperty.PropertyExtraData = pUCustomProperty->EProperty.ExtrasInfo;
		JsonMesh->CustomProperties.Add(customProperty);
	}

	//
	const TArray<UAssetUserData*>* AssetUserData = StaticMesh->GetAssetUserDataArray();

	for (UAssetUserData* pAssetData : *AssetUserData)
	{
		UCustomProperty* pCustomProperty = Cast<UCustomProperty>(pAssetData);
		if (pCustomProperty)
		{
			JsonMesh->ExtrasInfo = pCustomProperty->EProperty.ExtrasInfo;
		}
	}
	//
