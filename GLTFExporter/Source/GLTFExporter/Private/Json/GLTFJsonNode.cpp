...
{
	if (!ExtrasProperty.IsEmpty())
	{
		// write the extras that we already added to the mesh 
		Writer.Write(TEXT("extras"), ExtrasProperty);
	}
	...
	if (Mesh != nullptr)
	{
		...
		// write the extras that we already added to the mesh 
		//Writer.Write(TEXT("extras"), Mesh->CustomProperties);
		if (Mesh->CustomProperties.Num() > 0)
		{
			//Writer.Write(TEXT("extras"), CustomProperties);
			for (const FGLTFJsonCustomProperty& custProperty : Mesh->CustomProperties)
			{
				custProperty.WriteObject(Writer);
			}
	...
	// write Extras Data
	//Writer.Write(TEXT("Extras"), Children);
}
