...
public:
    ...
    void Write(const FString & identifier, const TSharedPtr<FJsonValue> iValue,int32 Number)
	{
		if (CurrentIdentifier.IsEmpty())
		{
			JsonWriter->WriteValue(Number);
	...
	{
		if (!CurrentIdentifier.Compare("extras"))
		{
			if (CurrentIdentifier.IsEmpty())
			{
			...
			{
				JsonWriter->WriteRawJSONValue(CurrentIdentifier, String);
				CurrentIdentifier.Empty();
			}
		}
		else
	...
	void WriteX(const FString& identifier,const FString& String)
	{
		if (CurrentIdentifier.IsEmpty())
		{
			JsonWriter->WriteRawJSONValue("extras", String);
			CurrentIdentifier = "extras";
		...
		{
			JsonWriter->WriteRawJSONValue(String);
			CurrentIdentifier.Empty(); // Sameer
		}
	}
