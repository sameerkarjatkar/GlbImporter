                                                          ...
                                                          GLTF::ETextureMode TextureMode)
{
	...
	{
		UE_LOG(LogTemp, Display, TEXT("Texture Name is Empty..skipping"));
		return nullptr;
	}

	
	// base64 issue
	if ((!GltfTexture.Source.URI.StartsWith(TEXT("data:image/jpeg;base64,"))
		)
		 &&
		(!GltfTexture.Source.URI.StartsWith(TEXT("data:image/png;base64,"))
			))

	{
		// Handle invalid format
		UE_LOG(LogTemp, Display, TEXT("Texture should be either jpeg/png"));
		return nullptr;
	}

	 FString TextureNm = ObjectTools::SanitizeObjectPath(FPaths::GetBaseFilename(GltfTexture.Name));

	//TODO
	//FString base64Data = GltfTexture.Source.URI.RightChop(22);
	//const char* charData = TCHAR_TO_UTF8(*base64Data);
	//TArray<uint8> decodedData;
	//uint8  dcdata = 0;
	//FBase64::Decode(*base64Data,base64Data.Len(), &dcdata);
	//FBase64::Decode(charData, decodedData);
	//FString filePath = FPaths::ProjectContentDir() + TEXT("my_texture.png");
	
	//
	 if (TextureName.Len() > 1023)
	 {
		 UE_LOG(LogTemp, Display, TEXT("Texture Name too long "));
		 return nullptr;
	 }
