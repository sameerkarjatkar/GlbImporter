...
namespace GLTF
{
	...
	void FFileReader::SetupMesh(const FJsonObject& Object) const
	{
		...
		// extras
		if (Object.HasField("extras"))
		{
			const TSharedPtr<FJsonObject> extrasField = Object.GetObjectField(TEXT("extras"));
			if (!extrasField->Values.IsEmpty())
			{
				Mesh.ExtrasInfo = extrasField;
				Asset->Metadata.extranfo = extrasField;
			}
		}
	...
	void FFileReader::SetupNode(const FJsonObject& Object) const
	{
		...
		if (Object.HasField(TEXT("extras")))
		{
			...
			//TMap<FString, TSharedPtr<FJsonValue>> objValues = Object.Values;
			const TArray<TSharedPtr<FJsonValue> >& ExtrasArray = Object.GetArrayField(TEXT("extras"));
			
			const FString& eField("extras");
			TSharedPtr<FJsonValue> fieldJson ;
			const TSharedPtr<FJsonObject>  extraObj =  Object.GetObjectField(eField);

			
			TMap<FString, TSharedPtr<FJsonValue>> extraValues = extraObj->Values;
			if (!extraValues.IsEmpty())
			{
				FMetadata extrasMetaData;
				TArray< GLTF::FMetadata::FExtraData> arrayExtraData;
			    for ( const TPair<FString,TSharedPtr<FJsonValue>> & Pair : extraValues)
				{
					GLTF::FMetadata::FExtraData extraData;
					FString label = Pair.Key;
					TSharedPtr <FJsonValue> PairVal =  Pair.Value;
					extraData.Name = label;
					extraData.Value = PairVal->AsString();
					
					extrasMetaData.Extras.Add(extraData);
					arrayExtraData.Add(extraData);
					Asset->Metadata.Extras.Emplace(FMetadata::FExtraData{ Pair.Key, PairVal->AsString() });
					Node.Extras.Add(Pair.Key, PairVal->AsString());
				}
				// set the extras json object
				Node.ExtrasInfo = extraObj;
			//	Asset->Metadata.Extras = arrayExtraData;
			...
			//for (TSharedPtr<FJsonValue> Value : extraValues)
			{
	...
	void FFileReader::SetupMaterial(const FJsonObject& Object) const
	{
		...
		FMetadata metadata = Asset->Metadata;
		...
		if (Object.HasTypedField<EJson::Object>(TEXT("extras")))
		{

			const FJsonObject& EXTRAS = *Object.GetObjectField(TEXT("extras"));
			const TSharedPtr<FJsonObject>  extraObj = Object.GetObjectField("extras");
			Material.ExtraProperty = extraObj;

			/*const TMap<FString, TSharedPtr<FJsonValue>>& ExtraValues = EXTRAS.Values;
			if (!ExtraValues.IsEmpty())
			{
				for (const auto& Pair : ExtraValues)
				{
					TArray<GLTF::FMaterial::FExtras> extraArray;
					GLTF::FMaterial::FExtras extraInfo;
					...
					if (Value.IsValid())
					{
						if (Value->Type == EJson::Number)
						{
							...
							FString StringValue = FString::SanitizeFloat(NumberValue);
							// Do something with the NumberValue
							extraInfo.ExtrasMap.Add(Pair.Key, StringValue);
							// ...
						}
						...
						extraArray.Add(extraInfo);
					//	Material.Extras.Add(extraArray);
					}
					Material.Extras = extraArray;
					
				//	Material.ExtraProperty = MakeShareable(new FJsonObject(EXTRAS));
				}
			}*/
		}
	...
	void FFileReader::ReadFile(const FString& InFilePath, bool bInLoadImageData, bool bInLoadMetadata, GLTF::FAsset& OutAsset)
	{
		...
		TSharedPtr<FJsonValue> ValField = JsonRoot->TryGetField(TEXT("Nodes"));
		// Assuming ValField is an array of objects
		if (ValField.IsValid() && ValField->Type == EJson::Array) {
			TArray<TSharedPtr<FJsonValue>> ArrayValues;
			ArrayValues = ValField->AsArray();

			for (TSharedPtr<FJsonValue> ArrayElement : ArrayValues) {
				if (ArrayElement.IsValid() && ArrayElement->Type == EJson::Object) {
					TSharedPtr<FJsonObject> ArrayObject;
					ArrayObject = ArrayElement->AsObject();

					// Now you can access the fields within each object
					TSharedPtr<FJsonValue> field = ArrayObject->TryGetField(TEXT("name"));
					if (field.IsValid()) {
						// Access the value in 'field' and process it as needed
					}
				}
			}
		}
