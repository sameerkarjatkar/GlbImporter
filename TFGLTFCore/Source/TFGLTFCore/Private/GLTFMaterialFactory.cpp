...
namespace GLTF
{
	...
	const TArray<FMaterialElement*>& FMaterialFactoryImpl::CreateMaterials(const GLTF::FAsset& Asset, UObject* ParentPackage, EObjectFlags Flags)
	{
		...
		for (const GLTF::FMaterial& GLTFMaterial : Asset.Materials)
		{
			...
			FMaterialElement* MaterialElement = nullptr;
			TArray<GLTF::FMaterial::FExtras> ExtraInfo = GLTFMaterial.Extras;
			// Need a better datastructure for Extras ?

			if (GLTFMaterial.ExtraProperty)
			{
				MaterialElement = MaterialElementFactory->CreateMaterial(*GLTFMaterial.Name, ParentPackage,
					Flags, GLTFMaterial.ExtraProperty);
			}

		/*	if (ExtraInfo.Num() > 0)
			{
				MaterialElement = MaterialElementFactory->CreateMaterial(*GLTFMaterial.Name, ParentPackage,
					Flags, ExtraInfo[0].ExtrasMap);
			}*/
			else
			{
				MaterialElement = MaterialElementFactory->CreateMaterial(*GLTFMaterial.Name, ParentPackage,
			...
			//MaterialElement->SetExtras(GLTFMaterial.Extras);
			//MaterialElement->Extras = ;
			// set Extras
			...
			// Add the Extra Info 
			//MaterialElement->Extras = GLTFMaterial.Extras;
