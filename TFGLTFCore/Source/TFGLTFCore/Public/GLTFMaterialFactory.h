...
namespace GLTF
{
	...
	class TFGLTFCORE_API ITextureFactory
	{
	public:
	...
	class TFGLTFCORE_API IMaterialElementFactory
	{
	public:
		virtual ~IMaterialElementFactory() = default;

		//virtual FMaterialElement* CreateMaterial(const TCHAR* Name, UObject* ParentPackage, EObjectFlags Flags,
			//                                     TMap<FString, FString> ExtrasMap = TMap<FString, FString>()) = 0;
		virtual FMaterialElement* CreateMaterial(const TCHAR* Name, UObject* ParentPackage, EObjectFlags Flags,
			                                     TSharedPtr<FJsonObject> ipExtrasProperty = TSharedPtr<FJsonObject>()) = 0;
	};

	class TFGLTFCORE_API FMaterialFactory
	{
	public:
