...
namespace GLTF
{
	...
	struct TFGLTFCORE_API FScene
	{
		FString       Name;
		TArray<int32> Nodes;
	};

	struct TFGLTFCORE_API FMetadata
	{
		struct FExtraData
		{
			FString Name;
		...
		TSharedPtr<FJsonObject> extranfo;
		const FExtraData* GetExtraData(const FString& Name) const;
	};

	struct TFGLTFCORE_API FAsset : FNoncopyable
	{
	...
	TFGLTFCORE_API const TCHAR* ToString(EExtension Extension);
