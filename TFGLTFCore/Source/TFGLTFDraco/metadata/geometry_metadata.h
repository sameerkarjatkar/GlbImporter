...
#include "TFGLTFDraco/metadata/metadata.h"


#ifdef DRACO_GEOMMETADATA_EXPORTS
#define DRACO_GEOMMETADATA_API __declspec(dllexport)
#else
#define DRACO_GEOMMETADATA_API __declspec(dllimport)
#endif
...
class DRACO_GEOMMETADATA_API GeometryMetadata : public Metadata {
 public:
  GeometryMetadata() {}
  ...
  const std::vector<std::shared_ptr<AttributeMetadata>> &attribute_metadatas()
      const {
    return att_metadatas_;
  }

 private:
  std::vector<std::shared_ptr<AttributeMetadata>> att_metadatas_;
