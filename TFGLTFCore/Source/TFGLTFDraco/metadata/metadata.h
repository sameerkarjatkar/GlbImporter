...
#ifdef DRACO_METADATA_EXPORTS
#define DRACO_METADATA_API __declspec(dllexport)
#else
#define DRACO_METADATA_API __declspec(dllimport)
#endif
...
class DRACO_METADATA_API EntryValue {
 public:
...
// metadata.
class DRACO_METADATA_API Metadata {
 public:
  Metadata() {}
  ...
  const std::map<std::string, std::shared_ptr<Metadata>> &sub_metadatas()
      const {
 ...
 private:
  ...
  std::map<std::string, std::shared_ptr<Metadata>> sub_metadatas_;
