...
//
#include "TFGLTFDraco/compression/attributes/kd_tree_attributes_decoder.h"

#include "TFGLTFDraco/compression/attributes/kd_tree_attributes_shared.h"
#include "TFGLTFDraco/compression/point_cloud/algorithms/dynamic_integer_points_kd_tree_decoder.h"
#include "TFGLTFDraco/compression/point_cloud/algorithms/float_points_tree_decoder.h"
#include "TFGLTFDraco/compression/point_cloud/point_cloud_decoder.h"
#include "TFGLTFDraco/core/draco_types.h"
#include "TFGLTFDraco/core/varint_decoding.h"
  ...
  const Self &operator=(const std::vector<CoeffT> &val) {
    for (auto index = 0; index < attributes_.size(); index++) {
      ...
      if (data_size < 4) {  // handle uint16_t, uint8_t
        ...
        for (uint32_t indexC = 0; indexC < num_components;
             indexC += 1, data_counter += data_size) {
          std::memcpy(data_counter, data_source + indexC, data_size);
        }
...
bool KdTreeAttributesDecoder::DecodeDataNeededByPortableTransforms(
  ...
  if (method == KdTreeAttributesEncodingMethod::kKdTreeQuantizationEncoding) {
    ...
    for (auto attribute_index = 0;
         static_cast<uint32_t>(attribute_index) < attribute_count;
         attribute_index += 1) {
      const int attr_id = GetAttributeId(attribute_index);
      PointAttribute *const attr =
          GetDecoder()->point_cloud()->attribute(attr_id);
...
bool KdTreeAttributesDecoder::TransformAttributesToOriginalFormat() {
  ...
  for (int i = 0; i < GetNumAttributes(); ++i) {
    ...
    if (att->data_type() == DT_INT32 || att->data_type() == DT_INT16 ||
      ...
      for (uint32_t ist = 0; ist < src_att->size(); ++ist) {
