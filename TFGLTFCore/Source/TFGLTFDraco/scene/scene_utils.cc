...
#include "TFGLTFDraco/core/draco_index_type_vector.h"
...
void SceneUtils::PrintInfo(const Scene &input, const Scene &simplified,
  ...
  struct Printer {
    Printer(const Scene &input, const Scene &simplified)
        : inputScene(input), isimplified(simplified), print_instanced_info(false) {
    ...
    void PrintAttInfoRow(const std::string &label, const draco::Scene &refinputScene,
                         const draco::Scene &refisimplified,
                         draco::GeometryAttribute::Type att_type) const {
      PrintInfoRow(label, NumAttEntriesOnBaseMeshes(refinputScene, att_type),
                   NumAttEntriesOnInstancedMeshes(refinputScene, att_type),
                   NumAttEntriesOnBaseMeshes(refisimplified, att_type),
                   NumAttEntriesOnInstancedMeshes(refisimplified, att_type));
    }

    const Scene &inputScene;
    const Scene &isimplified;
 ...
 private:
  ...
  void UpdateNodeIndices(Scene *scene) const {
    ...
    for (AnimationIndex i(0); i < scene->NumAnimations(); i++) {
      ...
      for (int iA = 0; iA < animation.NumChannels(); iA++) {
        const SceneNodeIndex node_index(animation.GetChannel(iA)->target_index);
        animation.GetChannel(iA)->target_index = node_map_[node_index].value();
      }
    }
    for (SkinIndex iS(0); iS < scene->NumSkins(); iS++) {
      Skin &skin = *scene->GetSkin(iS);
      for (int jS = 0; jS < skin.NumJoints(); jS++) {
        const SceneNodeIndex node_index = skin.GetJoint(jS);
        skin.GetJoint(jS) = node_map_[node_index];
      }
      const SceneNodeIndex root_index = skin.GetJointRoot();
      if (root_index != kInvalidSceneNodeIndex) {
