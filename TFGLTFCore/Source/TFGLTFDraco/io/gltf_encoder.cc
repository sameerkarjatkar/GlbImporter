...
//
#include "TFGLTFDraco/io/gltf_encoder.h"
...
#include "TFGLTFDraco/attributes/geometry_attribute.h"
#include "TFGLTFDraco/attributes/point_attribute.h"
#include "TFGLTFDraco/compression/draco_compression_options.h"
#include "TFGLTFDraco/compression/expert_encode.h"
#include "TFGLTFDraco/core/draco_types.h"
#include "TFGLTFDraco/core/vector_d.h"
#include "TFGLTFDraco/io/file_utils.h"
#include "TFGLTFDraco/io/file_writer_utils.h"
#include "TFGLTFDraco/io/gltf_utils.h"
#include "TFGLTFDraco/io/texture_io.h"
#include "TFGLTFDraco/mesh/mesh_features.h"
#include "TFGLTFDraco/mesh/mesh_splitter.h"
#include "TFGLTFDraco/mesh/mesh_utils.h"
#include "TFGLTFDraco/scene/instance_array.h"
#include "TFGLTFDraco/scene/scene_indices.h"
#include "TFGLTFDraco/scene/scene_utils.h"
...
Status GltfAsset::AddInstanceArrays(const Scene &scene) {
  ...
  for (InstanceArrayIndex i(0); i < scene.NumInstanceArrays(); ++i) {
    ...
    bool is_r_set = false;
    bool is_s_set = false;
    for (int iInstance = 0; i < array.NumInstances(); iInstance++) {
      const InstanceArray::Instance &instance = array.GetInstance(iInstance);
    ...
    for (int iNInstance = 0; i < array.NumInstances(); iNInstance++) {
      const InstanceArray::Instance &instance = array.GetInstance(iNInstance);
      if (is_t_set) {
  ...
  for (const PropertyTable *const table : property_tables_) {
    ...
    for (int i = 0; i < table->NumProperties(); ++i) {
      ...
      if (!property.GetArrayOffsets().data.data.empty()) {
        ...
        DRACO_ASSIGN_OR_RETURN(const int ibuffer_view_index,
                               AddBufferView(property.GetArrayOffsets().data));
        gltf_json_.OutputValue("arrayOffsets", ibuffer_view_index);
      }
      ...
      if (!property.GetStringOffsets().data.data.empty()) {
        ...
        DRACO_ASSIGN_OR_RETURN(const int obuffer_view_index,
                               AddBufferView(property.GetStringOffsets().data));
        gltf_json_.OutputValue("stringOffsets", obuffer_view_index);
      }
