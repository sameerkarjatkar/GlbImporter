...
//
#include "TFGLTFDraco/io/gltf_decoder.h"
...
#include "TFGLTFDraco/core/draco_types.h"
#include "TFGLTFDraco/core/hash_utils.h"
#include "TFGLTFDraco/core/status.h"
#include "TFGLTFDraco/core/status_or.h"
#include "TFGLTFDraco/io/tiny_gltf_utils.h"
#include "TFGLTFDraco/material/material_library.h"
#include "TFGLTFDraco/mesh/mesh.h"
#include "TFGLTFDraco/mesh/mesh_features.h"
#include "TFGLTFDraco/mesh/triangle_soup_mesh_builder.h"
#include "TFGLTFDraco/metadata/property_table.h"
#include "TFGLTFDraco/point_cloud/point_cloud_builder.h"
#include "TFGLTFDraco/scene/scene_indices.h"
#include "TFGLTFDraco/texture/source_image.h"
#include "TFGLTFDraco/texture/texture_utils.h"
...
Status GltfDecoder::AddStructuralMetadataToGeometry(GeometryT *geometry) {
  ...
  const tinygltf::Value::Object &oC = e->second.Get<tinygltf::Value::Object>();

  // Decode property table schema.
  {
    const auto &value = oC.find("schema");
    if (value == oC.end()) {
  ...
  {
    const auto &tables = oC.find("propertyTables");
    if (tables == oC.end()) {
      return ErrorStatus(
    ...
    for (int iP = 0; iP < tables_array.Size(); iP++) {
      // Create a property table and populate it below.
      std::unique_ptr<PropertyTable> property_table(new PropertyTable());

      const auto &objectProp = tables_array.Get(iP);
      if (!objectProp.IsObject()) {
        return ErrorStatus("Property table is malformed.");
      }
      const auto oA = objectProp.Get<tinygltf::Value::Object>();

      // The "class" property is required.
      bool success;
      std::string str_value;
      DRACO_ASSIGN_OR_RETURN(success, DecodeString("class", oA, &str_value));
      if (success) {
      ...
      int int_value;
      DRACO_ASSIGN_OR_RETURN(success, DecodeInt("count", oA, &int_value));
      if (success) {
      ...
      DRACO_ASSIGN_OR_RETURN(success, DecodeString("name", oA, &str_value));
      if (success) {
      ...
      // Decode property table properties (columns).
      {
        ...
        if (!objectProp.Has(kName)) {
          return ErrorStatus("Property table is malformed.");
        }
        const tinygltf::Value &valueObjProp = objectProp.Get(kName);
        if (!valueObjProp.IsObject()) {
          return ErrorStatus(
              "Property table properties property is malformed.");
        }

        // Loop over property table properties.
        for (const auto &keyVP : valueObjProp.Keys()) {
          // Create a property table property and populate it below.
          std::unique_ptr<PropertyTable::Property> propertyTab(
              new PropertyTable::Property());

          const auto &property_object = valueObjProp.Get(keyVP);
          if (!property_object.IsObject()) {
            return ErrorStatus("Property entry is malformed.");
          }
          propertyTab->SetName(keyVP);
          const auto oP = property_object.Get<tinygltf::Value::Object>();

          // The "values" property is required.
          DRACO_ASSIGN_OR_RETURN(
              success,
              DecodePropertyTableData("values", oP, &propertyTab->GetData()));
          if (!success) {
          ...
          DRACO_ASSIGN_OR_RETURN(
              success, DecodeString("stringOffsetType", oP, &str_value));
          if (success) {
            propertyTab->GetStringOffsets().type = str_value;
          }
          DRACO_ASSIGN_OR_RETURN(
              success, DecodeString("arrayOffsetType", oP, &str_value));
          if (success) {
            propertyTab->GetArrayOffsets().type = str_value;
          }
          DRACO_ASSIGN_OR_RETURN(
              success,
              DecodePropertyTableData("arrayOffsets", oP,
                                      &propertyTab->GetArrayOffsets().data));
          DRACO_ASSIGN_OR_RETURN(
              success,
              DecodePropertyTableData("stringOffsets", oP,
                                      &propertyTab->GetStringOffsets().data));

          // Add property to the property table.
          property_table->AddProperty(std::move(propertyTab));
        }
      }
...
Status GltfDecoder::DecodeMeshFeatures(
  ...
  const tinygltf::Value &earray = object->second;
  if (!earray.IsArray()) {
    return ErrorStatus("Mesh features array is malformed.");
  }
  for (int iM = 0; iM < earray.Size(); iM++) {
    ...
    const auto &objectRef = earray.Get(iM);
    ...
    {
      constexpr char okName[] = "featureCount";
      if (!objectRef.Has(okName)) {
        return ErrorStatus("Mesh features is malformed.");
      }
      const tinygltf::Value &value = objectRef.Get(okName);
      if (!value.IsInt()) {
    ...
    // All other properties are optional.
    {
      ...
      if (objectRef.Has(kName)) {
        const tinygltf::Value &value = objectRef.Get(kName);
        if (!value.IsInt()) {
    ...
    }
    {
      ...
      if (objectRef.Has(kName)) {
        const tinygltf::Value &valueLabel = objectRef.Get(kName);
        if (!valueLabel.IsString()) {
          return ErrorStatus("Label property is malformed.");
        }
        features.SetLabel(valueLabel.Get<std::string>());
      }
    }
    {
      constexpr char kName[] = "attribute";
      if (objectRef.Has(kName)) {
        const tinygltf::Value &valueAttr = objectRef.Get(kName);
        if (!valueAttr.IsInt()) {
          return ErrorStatus("Attribute property is malformed.");
        }
        features.SetAttributeIndex(valueAttr.Get<int>());
      }
    }
    {
      constexpr char kName[] = "texture";
      if (objectRef.Has(kName)) {
        const tinygltf::Value &valueTex = objectRef.Get(kName);
        if (!valueTex.IsObject()) {
        ...
        Material materialT(texture_library);
        const auto &container_object = objectRef.Get<tinygltf::Value::Object>();
        DRACO_RETURN_IF_ERROR(DecodeTexture(kName, TextureMap::GENERIC,
                                            container_object, &materialT));
        features.SetTextureMap(
            *materialT.GetTextureMapByType(TextureMap::GENERIC));

        // Decode array of texture channel indices.
        std::vector<int> channels;
        {
          constexpr char kNameCh[] = "channels";
          if (valueTex.Has(kNameCh)) {
            const tinygltf::Value &iarray = valueTex.Get(kNameCh);
            if (!iarray.IsArray()) {
              return ErrorStatus("Channels property is malformed.");
            }
            for (int iV = 0; iV < iarray.Size(); iV++) {
              const tinygltf::Value &ivalue = iarray.Get(iV);
              if (!ivalue.IsNumber()) {
                return Status(Status::DRACO_ERROR,
                              "Channels value is malformed.");
              }
              channels.push_back(ivalue.Get<int>());
            }
          } else {
            channels = {0};
          }
    ...
    }
    {
      ...
      if (objectRef.Has(kName)) {
        const tinygltf::Value &valueP = objectRef.Get(kName);
        if (!valueP.IsInt()) {
          return ErrorStatus("Property table property is malformed.");
        }
        features.SetPropertyTableIndex(valueP.Get<int>());
      }
    }
  }
  return OkStatus();
...
StatusOr<bool> GltfDecoder::CheckKhrTextureTransform(
  ...
  const auto &eT = extension.find("KHR_texture_transform");
  if (eT == extension.end()) {
    return false;
  }
  const tinygltf::Value::Object &oV = eT->second.Get<tinygltf::Value::Object>();
  const auto &scale = oV.find("scale");
  if (scale != oV.end()) {
    const tinygltf::Value &arrayScale = scale->second;
    if (!arrayScale.IsArray() || arrayScale.Size() != 2) {
      return Status(Status::DRACO_ERROR,
                    "KhrTextureTransform scale is malformed.");
    }
    std::array<double, 2> iscale;
    for (int iScale = 0; iScale < arrayScale.Size(); iScale++) {
      const tinygltf::Value &valueSc = arrayScale.Get(iScale);
      if (!valueSc.IsNumber()) {
        return Status(Status::DRACO_ERROR,
                      "KhrTextureTransform scale is malformed.");
      }
      iscale[iScale] = valueSc.Get<double>();
      transform_set = true;
    }
    transform->set_scale(iscale);
  }
  const auto &rotation = oV.find("rotation");
  if (rotation != oV.end()) {
    const tinygltf::Value &valueRot = rotation->second;
    if (!valueRot.IsNumber()) {
      return Status(Status::DRACO_ERROR,
                    "KhrTextureTransform rotation is malformed.");
    }
    transform->set_rotation(valueRot.Get<double>());
    transform_set = true;
  }
  const auto &offset = oV.find("offset");
  if (offset != oV.end()) {
    const tinygltf::Value &oarray = offset->second;
    if (!oarray.IsArray() || oarray.Size() != 2) {
      return Status(Status::DRACO_ERROR,
                    "KhrTextureTransform offset is malformed.");
    }
    std::array<double, 2> Toffset;
    for (int iN = 0; iN < oarray.Size(); iN++) {
      const tinygltf::Value &valueN = oarray.Get(iN);
      if (!valueN.IsNumber()) {
        return Status(Status::DRACO_ERROR,
                      "KhrTextureTransform offset is malformed.");
      }
      Toffset[iN] = valueN.Get<double>();
      transform_set = true;
    }
    transform->set_offset(Toffset);
  }
  const auto &tex_coord = oV.find("texCoord");
  if (tex_coord != oV.end()) {
